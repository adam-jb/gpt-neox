...
def generate_samples_from_prompt_stream2(
    ...
    stop_tokens=None,
 	...
 	# generate completions
 	generated_texts = []
 	for i in range(1):  # 10 tokens returned CHANGE THIS HARD CODED LIMIT!!
 		model.module.clear_cache()  # clear kv cache between batches
...
if __name__ == '__main__':
	app.run(threaded=True, host='209.137.198.4', port=5000, debug=False) # threaded=True mode allows concurrent requests, opening a new thread for each new request; but might block gunicorn
